...
void tambah_stok(const char *kode_barang, int jumlah_tambahan) {
    for (int i = 0; i < jumlah_barang; i++) {
        if (strcmp(daftar_barang[i].kode, kode_barang) == 0) {
            daftar_barang[i].stok += jumlah_tambahan;
            printf("Stok barang dengan kode %s berhasil ditambahkan sebanyak %d.\n", kode_barang, jumlah_tambahan);
            return;
        }
    }
...
void ubah_harga(const char *kode_barang, double harga_baru) {
    for (int i = 0; i < jumlah_barang; i++) {
        if (strcmp(daftar_barang[i].kode, kode_barang) == 0) {
            daftar_barang[i].harga = harga_baru;
            printf("Harga barang dengan kode %s berhasil diubah menjadi %.2lf.\n", kode_barang, harga_baru);
            return;
        }
    }
...
void laporan_stok() {
    ...
    printf("---------------------------------------------\n");
    printf("Kode    Nama Barang              Stok\n");
    printf("---------------------------------------------\n");

    for (int i = 0; i < jumlah_barang; i++) {
        printf("%-8s%-24s%-8d\n", daftar_barang[i].kode, daftar_barang[i].nama, daftar_barang[i].stok);
    }
